...
const ImageStatisticsModal: FC<Props> = ({ stats, onClose }) => {
  ...
  return (
    <div style={{
      ...
      {gridView &&
        <div className="af-form-field" style={{ padding: '10px', marginTop: '10px', maxHeight: '70vh', overflow: 'auto' }}>
          <Grid container spacing={2} columns={{ xs: 4, sm: 6, md: 8, xl: 12 }}>
            {
              stats.map((stat, index) => {
                ...
                return <Grid item xs={2} key={index}>
                  <Card>
                    <a href={stat.src} title={stat.src} target="_blank" rel="noreferrer">
                      <CardMedia
                        sx={{ aspectRatio: '1' }}
                        image={stat.src}
                      />
            ...
            <TableBody>
              {
                stats.map((stat, index) => {
                  ...
                  return (
                    <TableRow key={index}>
                      <TableCell>
                        <a href={stat.src} target="_blank" rel="noreferrer" title={stat.src}>
