...
const Text: FC<Props> = ({ text = [], align = 'left' }) => {
  const options = {
    overrides: {
    ...
    <>
      {text.map((item: any) => {
        ...
        switch (type) {
          case 'markdown':
            return (
              <Box key={nanoid()} className="amp-dc-text" style={{ textAlign: align, padding: '10px 0' }}>
            ...
            return (
              data && (
                <picture key={data.name} className="amp-dc-image">
                  <img src={getImageURL(data, { strip: true })} className="amp-dc-image-pic" alt={data.name} />
                </picture>
              )
            );
          default:
