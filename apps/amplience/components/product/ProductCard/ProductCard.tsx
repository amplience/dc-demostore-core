...
const ProductCardSkeleton: React.FC<Props> = (props) => {
  ...
  if (variant.images) {
    if (variant.images[0] && variant.images[0].url) {
      firstImage = variant.images[0].url.replace(
        "i8.amplience.net",
        "cdn.media.amplience.net"
      );
      if (firstImage.indexOf("cdn.media.amplience.net") > 0) {
        firstImage +=
          "?fmt=auto&qlt=default&fmt.jpeg.qlt=75&fmt.webp.qlt=60&fmt.jp2.qlt=40&w=540&upscale=false";
      }
    }
    if (variant.images[1] && variant.images[1].url) {
      secondImage = variant.images[1].url.replace(
        "i8.amplience.net",
        "cdn.media.amplience.net"
      );
        ...
        <div className={clsx(classes.root, className)} {...other}>
          ...
          <div className={classes.details}>
            ...
            {(!variant.salePrice ||
              ...
              _.isEmpty(variant.salePrice)) &&
              variant.listPrice}
            {variant.salePrice && variant.listPrice !== variant.salePrice && (
              <div>
                <span className={classes.salePrice}>{variant.listPrice}</span>{" "}
                {variant.salePrice}
              </div>
            )}
          </div>
