...
export const WithLazyAppContext: React.FC<Props> = (props) => {
...
export const WithAppContext: FC<{
  value: DemoStoreConfiguration;
...
export const configLocator =
  process.env.NEXT_PUBLIC_DEMOSTORE_CONFIG_LOCATOR ||
  process.env.STORYBOOK_DEMOSTORE_CONFIG_LOCATOR ||
  `amprsaprod:default`;
export async function createAppContext(): Promise<DemoStoreConfiguration> {
  ...
  if (typeof context.cms.hub === "object") {
    context.cms = {
      ...
      imageHub: (context.cms as any).hubs.find(
        (hub: any) => hub.key === "productImages"
      )?.name,
    };
  }
